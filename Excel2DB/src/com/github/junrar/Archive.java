...
public class Archive implements Closeable {
	 ...
	 * @throws RarException
	 */
	public void extractFile(FileHeader hd, OutputStream os, boolean checkCrc) throws RarException {
		if (!headers.contains(hd)) {
			throw new RarException(RarExceptionType.headerNotInArchive);
		}
		try {
			doExtractFile(hd, os, checkCrc);
		} catch (Exception e) {
	...
	private void doExtractFile(FileHeader hd, OutputStream os, boolean checkCrc)
			throws RarException, IOException {
		dataIO.init(os);
		dataIO.init(hd);
